...
2. scans tracked and non-ignored untracked candidate inputs, then creates a temporary build context
   containing only the required tracked Desktop/package/protocol/build files; the Dockerfile applies
   a second explicit `COPY` whitelist before any cache export. The protocol packages live in this
   repository's own `packages/` (upstream removed the `cindy-protocol` submodule in 2026-08), so
   there is no separate submodule source validation or gitlink fetch;
3. builds the `runtime` target as `linux/amd64`, loads it only into the job-local Docker daemon, and
   never logs in to a registry;
4. runs capability, health, non-root entrypoint, packaged Electron startup/SIGTERM and
...
5. exports a short-lived gzip-compressed Docker image archive, archive SHA-256, tested local image
   config ID, a build/load digest with its exact source field, an SPDX image SBOM, and a Trivy
   Critical report;
6. fails on fixable Critical vulnerabilities.
