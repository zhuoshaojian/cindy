...
describe('runInvoke 双层校验', () => {
  it('routes OAuth with the authenticated src and applies revocation before its private handler', async () => {
    ...
    }, () => ({deviceId:'cloud',membershipId:'member'}));
    try {
      ...
      const exchange=await authenticateOauthController({
        target:{...publishedPluginOauthIdentity()!,instanceId:'instance',deviceId:'cloud',membershipId:'member',observedAtMs:now,expiresAtMs:now+60000},
        peer:'ctrl-a',action,ghostId:'p',assertCurrent:()=>{},
