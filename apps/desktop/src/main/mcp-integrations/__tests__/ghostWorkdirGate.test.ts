...
vi.mock('../../cindy-brain/index.js', () => ({
  connectGhostGithubAccount: githubConnectMock,
...
beforeEach(() => {
  ...
  githubConnectMock.mockReset();
  githubConnectMock.mockResolvedValue(null);
  listMock.mockReset();
...
describe('connect_account ordinary task entry', () => {
  it('routes official GitHub login through the Host adapter from an ordinary task', async () => {
    listMock.mockReturnValue([chipGhost('cindy-github')]);
    githubConnectMock.mockResolvedValueOnce({ ok: true, status: 'ready', ghostId: 'cindy-github' });
    ...
    expect(githubConnectMock).toHaveBeenCalledWith(expect.objectContaining({ signal, reauthorize: true, assertCurrent: expect.any(Function) }));
    expect(ensureReadyMock).not.toHaveBeenCalled();
    expect(dispatchMock).not.toHaveBeenCalled();
    expect(grantAttachmentsMock).not.toHaveBeenCalled();
  });
  it('blocks disabled GitHub before the login helper and does not bypass trust rejection', async () => {
    listMock.mockReturnValue([chipGhost('cindy-github')]);
